...
- Un site vitrine / landing page sans backend → trop lourd, utilise `static-template` à la place
...
```bash
# Sur le VPS, dans /home/antares/deploy/
# new-site.sh copie le template (sans .git) et lance init.sh automatiquement
...
> `app-template` est un dépôt git. Un `cp -r` copierait le dossier `.git`
> et le nouveau projet hériterait de l'historique du template — ce qu'on ne veut pas.
> `new-site.sh` utilise `rsync --exclude='.git'` pour copier uniquement les fichiers.
...
0 2 * * * cd /home/antares/deploy/dupont && ./scripts/db-backup.sh >> /var/log/backup-dupont.log 2>&1
```
...
- [ ] `.env` existe et ne contient plus de valeur `GENERATED` ou `CLIENT_NAME`
